...
async def create_resume(user_id: str = Depends(get_supabase_user), file: UploadFile = File(...)):
    """
    Insert Resume Data
    """
    try:
...
async def get_resumes(user_id:str = Depends(get_supabase_user)):
    return ResumeCollection(users=await resume_collection.find({"user_id": user_id}).to_list(100))  # Ensure correct serialization
...
async def show_resume_by_id(id: str):
    """
    Get the record for a specific resume, looked up by `id`.
    """
    if (resume := await resume_collection.find_one({"_id": ObjectId(id)})) is not None:
        return resume
...
async def update_resume(id: str, resume: ResumeUpdateModel = Body(...)):
    ...
    if (existing_resume := await resume_collection.find_one({"_id": ObjectId(id)})) is not None:
...
async def delete_resume(id: str):
    ...
    """
    delete_result = await resume_collection.delete_one({"_id": ObjectId(id)})
